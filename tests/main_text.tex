...
These improved results allow us to make robust and precise statements on the dynamical nonlinear tails. In the second part, we study the dynamical formation of nonlinear tails in a scalar field model with a cubic $\lambda \Phi^3$ coupling via both analytic and numerical means.
...
\begin{align}
  ...
  G_Q(t,r_*;t',r_*') &\approx \Re \left[ \sum_n B_n e^{s_n (t-t'-r_*-r_*')} \right]
    \nonumber \\
  G_B(t,r_*;t',r_*') &\approx (-1)^{l+1} \frac{2(2l+2)!}{[(2l+1)!!]^2} (rr')^{l+1} (t-t')^{-2l-3} \;.
\end{align}
In $G_Q$, the sum is over poles of $G(s,r_*,r_*')$, $\omega_n = i s_n$'s are the quasinormal frequencies, and the $B_n$'s are quasinormal excitation factors. In $G_B$, the $(t-t')^{-2l-3}$ yields the well known Price's power law.
...
\begin{align}
  G_Q(t,r_*;t',r_*') &\approx \Re \left[ \sum_n B_n e^{s_n (t-v'-r_*)} \right] \;.
\end{align}
...
\begin{align}
  K(t) &\approx \frac{-1}{l+2-\beta} \left(\eval{r'}_{r_*'=r_i}\right)^{l+2-\beta}  \;.
\end{align}
